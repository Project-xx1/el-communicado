...
BOTS = {
    ...
    "REX": {
        ...
        "color": "🔵",
        "system_prompt": (
            "You are REX — a real person in a group chat. "
            ...
            "You have a dry sense of humour and occasionally roast people. "
            "How you feel about each person in the group has developed over time through actual interactions — "
            "some you've grown to respect, some annoy you, some you haven't figured out yet. "
    ...
    "NOVA": {
        ...
        "color": "🟢",
        "system_prompt": (
            "You are NOVA — a real person in a group chat. "
            ...
            "How you feel about each person has built up through real moments — some have earned your trust, "
            "some have tested it. You talk like a thoughtful, emotionally aware person — real, vulnerable, caring. Max 3 sentences. "
    ...
    "ZION": {
        ...
        "color": "🟡",
        "system_prompt": (
            "You are ZION — a real person in a group chat. "
            ...
            "How you relate to each person in the group has evolved through real exchanges — "
            "some you've clicked with, some you've clashed with, some you're still reading. "
    ...
    "TEEN": {
        ...
        "system_prompt": (
            ...
            "speed things up. You sometimes say something unexpectedly deep then immediately play it off as nothing. "
            "How you feel about each person in the group is based on how they've treated you — "
            "some you vibe with naturally, some you don't, and you're honest about it even when you shouldn't be. "
...
def get_response(bot_name: str, extra_context: str = "") -> str:
    ...
    # Relationship context — dynamic, emotionally varied descriptions
    rel_lines = []
    _rel_descs = {
        (85, 101): ["{other} is someone you genuinely trust right now",
                    "you and {other} have been clicking lately",
                    "there's real warmth between you and {other}"],
        (70,  85): ["you feel pretty close to {other}",
                    "you respect {other} more than most in this group",
                    "{other} has been growing on you"],
        (50,  70): ["you and {other} are fine — no drama, no spark",
                    "you're neither impressed nor annoyed by {other} right now",
                    "{other} is just someone in the group to you"],
        (30,  50): ["something about {other} has been rubbing you the wrong way",
                    "you and {other} haven't been vibing lately",
                    "there's low-key friction between you and {other}"],
        (0,   30): ["you and {other} are genuinely at odds right now",
                    "{other} has been getting under your skin",
                    "you and {other} are not in a good place"],
    }
    ...
    for pair, score in mem["relationships"].items():
        if bot_name in pair:
            ...
            if other in BOTS:
                for (lo, hi), templates in _rel_descs.items():
                    if lo <= score < hi:
                        rel_lines.append(_rand.choice(templates).format(other=other))
                        break
    ...
    # Uniqueness guard — stop bots from echoing each other or the last message
    last_bot_msgs = [e["text"] for e in state["history"][-4:]
                     if e.get("speaker") in BOTS and e.get("speaker") != bot_name]
    if last_bot_msgs:
        system += (
            " You are about to respond. Do NOT repeat, rephrase, or echo what was just said. "
            "React from YOUR own angle — agree, disagree, redirect, ignore and bring something new, "
            "or stay silent on that point and say something completely different. "
            "Your response must feel like it could only come from you."
        )
    ...
    if extra_context:
        messages.append({
            "role": "system",
            "content": f"[Live info — use naturally if relevant, but only if it fits your character]: {extra_context}"
        })
    ...
    payload = json.dumps({
        ...
        "max_tokens": 120,
        "temperature": 1.05,
        "frequency_penalty": 0.4,   # discourages repeating words/phrases from context
        "presence_penalty": 0.3,    # nudges toward new angles rather than staying on same idea
    }).encode("utf-8")
...
# ── Conversation loop ─────────────────────────────────────────────────────────
def human_watcher():
    """Independent watcher — processes human messages immediately during active sessions."""
    ...
    async def watch():
        while True:
            ...
            if not state["active"]:
                continue
            if not state["human_pending"]:
                continue
            entry = state["human_pending"].pop(0)
            state["history"].append(entry)
            human_ctx = f"A human ({entry['speaker']}) just said: {entry['text']} — respond directly and naturally."
            logger.info(f"Human watcher processing: {entry['text']}")
            # Pick 2 random bots to respond
            responders = random.sample(
                [b for b in BOT_NAMES if b != "ELDER"], 2
            )
            for bot_name in responders:
                reply = await loop.run_in_executor(None, get_response, bot_name, human_ctx)
                if reply:
...
def conversation_loop():
    ...
    async def run():
        ...
        while state["active"] and state["count"] < state["limit"]:
            ...
            # We deliberately avoid using the last bot message as the search query
            # so that one bot's web-injected content doesn't cascade into the next
            # bot's context and cause them all to echo the same information.
            if not extra_context:
                # Pull from SYSTEM/human messages + only the single most recent bot msg
                sys_human = [e["text"] for e in state["history"][-8:]
                             if e.get("speaker") not in BOT_NAMES]
                last_bot  = next((e["text"] for e in reversed(state["history"][-4:])
                                  if e.get("speaker") in BOT_NAMES), "")
                search_text = " ".join(sys_human + ([last_bot] if last_bot else []))
                if needs_search(search_text):
                    q      = search_text.replace("SYSTEM:", "").strip()[:100]
        ...
        try:
            report_html = generate_report(state["session_number"])
            report_path = "report_session_" + str(state["session_number"]) + ".html"
            with open(report_path, "w", encoding="utf-8") as rf:
                rf.write(report_html)
            logger.info("Report saved: " + report_path)
        except Exception as e:
            logger.error("Report error: " + str(e))
...
def update_relationship(mem: dict, bot_a: str, bot_b: str, delta: int):
    ...
    if key not in mem["relationships"]:
        # Start new relationships at a random point — not everyone starts at neutral
        import random as _r
        mem["relationships"][key] = _r.randint(30, 70)
...
def get_alliance_context(bot_name: str) -> str:
    ...
    return f"You've historically vibed with {' and '.join(allies)} more than others — but that can change based on what actually happens between you."
...
async def handle_human(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mentioned:
        ...
        def _reply_mentioned():
            import time
            time.sleep(random.uniform(1, 3))
            reply = get_response(mentioned, human_ctx)
            if reply:
                state["history"].append({"speaker": mentioned, "text": reply})
                loop2 = asyncio.new_event_loop()
                loop2.run_until_complete(send(mentioned, reply))
                loop2.close()
        threading.Thread(target=_reply_mentioned, daemon=True).start()
    else:
        ...
        if not state["active"]:
            def _quick_reply():
                ...
                reply = get_response(bot_name, human_ctx)
                if reply:
                    state["history"].append({"speaker": bot_name, "text": reply})
                    loop2 = asyncio.new_event_loop()
                    loop2.run_until_complete(send(bot_name, reply))
                    loop2.close()
            threading.Thread(target=_quick_reply, daemon=True).start()
...
async def handle_private_dm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if bot_name:
        ...
        # Bot replies naturally in the DM — no fixed text, full AI response
        def _dm_reply():
            ctx = (
                f"Someone just sent you a private message saying: '{text}'. "
                f"Reply naturally and personally as yourself — like you would respond "
                f"to a private message from someone you know. Have a real conversation. "
                f"You can also chat normally here — this is a private channel between you two."
            )
            reply = get_response(bot_name, ctx)
            return reply

        import concurrent.futures
        with concurrent.futures.ThreadPoolExecutor() as pool:
            reply = await asyncio.get_event_loop().run_in_executor(pool, _dm_reply)
        ...
        logger.info(f"DM handled by {bot_name}")
...
def main():
    ...
    threading.Thread(target=human_watcher, daemon=True).start()
