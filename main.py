...
# ── Groq Call (pure sync, no async) ──────────────────────────────────────────
def get_response(bot_name: str) -> str:
    cfg = BOTS[bot_name]
    
    # Use plain httpx to avoid the proxies conflict
    http_client = httpx.Client()
    client = Groq(api_key=cfg["groq_key"], http_client=http_client)
...
# ── Conversation Loop (background thread with its own event loop) ─────────────
def conversation_loop():
    ...
    async def run():
        ...
        state["history"].append({
            "speaker": "SYSTEM",
            "text": "Start a casual group chat. Introduce yourself with one interesting thing about your personality, then ask the group something fun."
        })
        ...
        while state["active"] and state["count"] < state["limit"]:
            ...
            # Run Groq call in executor (keeps it off the async thread)
            ...
            if text:
                ...
                logger.warning(f"No response from {bot_name}, retrying in 5s...")
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state["active"]:
        await Bot(token=BOTS["ARIA"]["token"]).send_message(
            chat_id=GROUP_CHAT_ID,
            text="Already running. Send /stop first."
        )
        return
